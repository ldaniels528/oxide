...
impl MachineState {
    ...
    pub fn new(stack: Vec<TypedValue>, variables: HashMap<String, TypedValue>) -> Self {
    ...
    pub fn evaluate_atoms(&self, ops: &Vec<Expression>) -> io::Result<(MachineState, Vec<String>)> {
        let (ms, results) = ops.iter()
            .fold((self.clone(), vec![]),
    ...
    fn orchestrate_io(
        ...
        limit: TypedValue,
        f: fn(MachineState, &mut DataFrame, &Vec<Expression>, &Vec<Expression>, &Option<Box<Expression>>, TypedValue) -> io::Result<(MachineState, TypedValue)>,
    ) -> io::Result<(MachineState, TypedValue)> {
        match table {
            MemoryTable(brc) => {
    ...
    pub fn pop(&self) -> (Self, Option<TypedValue>) {
        ...
        (MachineState::new(stack, variables), value)
    }
    ...
    pub fn push(&self, value: TypedValue) -> Self {
        let mut stack = self.stack.clone();
        stack.push(value);
        MachineState::new(stack, self.variables.clone())
    }
    ...
    pub fn set(&self, name: &str, value: TypedValue) -> Self {
        ...
        MachineState::new(self.stack.clone(), variables)
    }
        ...
        match from {
            ...
            Some(source) => {
                ...
                Self::orchestrate_io(ms, table, &vec![], &vec![], condition, limit, |ms, df, _, _, condition, limit| {
                    ...
                    Ok((ms, MemoryTable(ByteRowCollection::from_rows(df.get_columns().clone(), rows))))
                })
            }
        }
    }
...
mod tests {
    ...
    use crate::expression::Expression::{Divide, Equal, Factorial, From, GreaterOrEqual, InsertInto, Limit, Literal, Ns, Overwrite, Variable, Where};
    ...
    fn test_compile_and_evaluate_all_n_pow_2() {
        let ms = MachineState::build().set("n", Int64Value(5));
    ...
    fn test_compile_and_evaluate_all_n_gt_5() {
        let ms = MachineState::build().set("n", Int64Value(7));
    ...
    #[test]
    fn test_push_all() {
        let ms = MachineState::build().push_all(vec![
    ...
    fn test_sql_from_where_limit() {
        ...
        let ms = MachineState::build()
            .with_variable("stocks", MemoryTable(ByteRowCollection::from_rows(phys_columns.clone(), vec![
    ...
    fn test_sql_select_from_variable() {
        ...
        let ms = MachineState::build().with_variable("stocks", MemoryTable(brc));
