...
use crate::expression::{Conditions, Expression, UNDEFINED};
...
pub fn eval_pull_row(
    ...
    let (from, condition, limit) =
        compiler::get_sql_update_constraints(source.clone(), |from, condition, limit| {
    ...
    let row = df.pop_row(df.get_parameters());
...
pub fn do_table_query(
    ...
    fn unwind(
        ...
        match from.clone() {
            GroupBy { from, columns: group_by } =>
                unwind(ms, from.deref(), fields, condition, Some(group_by), having, order_by, limit),
            Having { from, condition } =>
                unwind(ms, from.deref(), fields, Some(Condition(condition)), group_by, having, order_by, limit),
            Limit { from, limit } =>
                unwind(ms, from.deref(), fields, condition, group_by, having, order_by, Some(limit)),
            OrderBy { from, columns: order_by } =>
                unwind(ms, from.deref(), fields, condition, group_by, having, Some(order_by), limit),
            Select { fields, from, .. } =>
                unwind(ms, &from.unwrap_or(UNDEFINED.into()), fields, condition, group_by, having, order_by, limit),
            Where { from, condition } =>
                unwind(ms, from.deref(), fields, Some(Condition(condition)), group_by, having, order_by, limit),
            // execute the query
            _ => {
    ...
    unwind(ms, query_comp, vec![], None, None, None, None, None)
}
...
mod tests {
    ...
    fn test_referenced_embedded_table() {
        ...
        interpreter = verify_exact_code_with(interpreter, r#"
            { last_sale: 11.68, processed_time: 2025-01-13T03:25:49.120Z } ~> history
        "#, "1");
